...
<body class="flex flex-col items-center justify-center select-none">
  ...
  <form id="submitForm" method="POST" action="{{ route('level.submit', $id) }}" style="display:none;">
    @csrf
    <input type="hidden" name="benar" id="inputBenar">
  ...
  <script>
    ...
    if (stars === 0) {
      ...
      feedbackText = "📘 Coba Belajar Lagi!";
      feedbackColor = "text-yellow-400";
    } else if (stars === 2) {
      feedbackText = "💪 Baik! Tetap Belajar!";
      feedbackColor = "text-blue-300";
    } else if (stars === 3) {
      feedbackText = "🏆 Kamu Hebat!";
      feedbackColor = "text-green-400";
    }

    // Tambahkan teks hasil besar
    ...
    // Isi data hasil ke input form tersembunyi
    document.getElementById("inputBenar").value = correctCount;
    document.getElementById("inputExp").value = exp;
    document.getElementById("inputJawaban").value = JSON.stringify(answers);

    // Submit form
    document.getElementById("submitForm").submit();
    };

    // 🔹 Tambahkan tombol “Kembali ke Peta Level”
    const backBtn = document.createElement("button");
    backBtn.textContent = "⬅️ Kembali ke Peta Level";
    backBtn.className =
      "mt-6 px-6 py-3 bg-gray-700 text-white font-semibold rounded-xl hover:bg-gray-600 transition-all duration-200 shadow-lg";
    backBtn.onclick = () => {
      window.location.href = "http://localhost/waracademy-main/public/level";
    };

    // Masukkan tombol ke dalam resultBox
